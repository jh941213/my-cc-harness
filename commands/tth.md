---
description: "TTH 멀티 에이전트 사일로 — M7 CEO 팀 자율 협업 (Toss 사일로 + Musk 5-Step + Ralph Loop)"
argument-hint: "[프로젝트/작업 설명]"
---
...
스폰 시 각 팀원에게 전달할 내용 (첫 브리핑에 전체 컨텍스트를 담는다 — 재브리핑 왕복은 비용):
```
...
> **Anthropic 인사이트**: "하네스의 모든 구성요소는 '모델이 혼자 못 한다'는 가정을 인코딩한다. 모델이 바뀔 때마다 그 가정을 스트레스 테스트하라." Evaluator는 고정 yes/no가 아니다 — 모델이 혼자 잘 할 수 있는 범위면 불필요한 오버헤드다. (Opus 5는 잘 정의된 첫 지시만으로 장시간 자율 실행 + 자가 검증이 기본 — 단, 생성자-평가자 분리는 자기 채점 편향 때문에 여전히 유효)
