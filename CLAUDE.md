...
- **Research First**: 해결책을 설계하기 전에 이미 자리 잡은 제품·이 코드베이스가 같은 문제를 어떻게 푸는지 먼저 조사. 접근을 발명하지 말고 검증된 패턴을 채택
- **삭제 우선**: 호환 레이어·폴백·`_v2`를 덧붙이는 대신 쓰이지 않는 경로를 삭제 (공개 API/SDK 프로젝트는 프로젝트 CLAUDE.md에 반대로 명시)
- **레이어로 성장**: 엔드투엔드로 동작하는 최소 버전에서 시작해 동작하는 것 위에 하나씩. 동작하는 코드를 미완성 복잡도와 맞바꾸지 않는다
- **기존 의존성 먼저**: 직접 구현·패키지 추가 전에 설치된 의존성 확인. 문서·타입을 보지 않고 "그 기능은 없다"고 단정 금지
...
- 단독 세션 장기 작업: 완료 조건이 단일하면 내장 `/goal <검증 가능한 완료 조건>`, PRD 기반 다항목·품질 게이트가 필요하면 autodev 스킬 (멀티에이전트 장기 작업은 /tth의 Ralph Loop)
...
### 컨텍스트 파일 운영
**이 파일은 지식 저장소가 아니라 편향 교정 프롬프트다. 품질은 길이가 아니라 밀도로 결정된다.**
- 에이전트가 스스로 알아낼 수 있는 정보(디렉토리 구조·설치 패키지·코드 스타일)는 적지 않는다 — 탐색하거나 auto-memory로 라우팅
- 같은 유형의 수정을 3회 반복 지시받으면 `tasks/lessons.md`의 해당 패턴을 이 파일의 한 줄로 승격한다
- 분기마다 harness-audit의 A/B/C 프루닝 실행: 스스로 알 수 있는 줄→삭제, 기본 동작의 반대를 지시하는 줄→유지, 팀 선택을 적은 줄→최신인지 확인. 더 이상 발동하지 않는 줄은 삭제한다
